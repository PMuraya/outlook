    ...
    implements mod.questionnaire
{
    ...
    *get_subscription_data(): Generator<quest.label>{
    ...
    async check(): Promise<boolean> {
        ...
        const business:outlook.business = this.get_business();
        //
    ...
    get_business(): outlook.business {
        ...
        if(select === '0') {
            ...
            return {source: 'user', id, name};
        }
        //
        //Else return the pk of the selection.
        else{
   ...
   //Fill the user roles with the roles from the database. 
    fill_user_roles() {
        ...
        inputs.forEach(input =>{
            ...
            const label = this.create_element(elem, "label", {textContent:input.key});
            //
            //Create a new input element and add the attributes(inputs)
            const role = this.create_element(label, "input" , { type:"checkbox", name:'roles' ,id: input.key, value:input.value});
            //
...
class register_business extends outlook.popup<outlook.business>{
    ...
    async check(): Promise<boolean> {
        ...
        //
        //Check the source of the business.
        if(business.source === 'user') {
            //
            ...
            this.result ={source:"user",id, name};
            //
        } else{
            const pk:string = this.get_selected_value('business');
            //
            //Initialize the result
            this.result = {source:"selector", pk};
        }
        //
    ...
    *create_business(): Generator<quest.layout>{
    ...
    async show_panels() {
        ...
        if (this.business!.source === "selector" || this.business!.source === "user") 
            alert
